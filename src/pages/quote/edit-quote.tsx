...
export default function EditQuote() {
  const form = useForm<SalesOrderFormData>({
    ...
    defaultValues: {
      customerId: "",
      items: [{ productId: "", productName: "", quantity: 1, unitPrice: 0, installmentPrice: 0 }],
  ...
  useEffect(() => {
    if (location.state) {
      ...
      form.reset({
        customerId: "",
        items: state.items || [
          { productId: "", productName: "", quantity: 1, unitPrice: 0, installmentPrice: 0 },
        ],
      ...
      setLoading(true);
      orderApi.findOne(id).then((resp) => {
        const order = resp.data;
        if (order.status !== OrderStatusEnum.DRAFT) {
          toast.error("Apenas orçamentos podem ser editados.");
          navigate("/quotes");
          return;
        }

        setSelectedClient(order.client ? { name: order.client.name, id: order.client.id } as any : null);

        form.reset({
          customerId: order.client ? order.client.id.toString() : "",
          ...
          includeDelivery: !!order.shippingCost,
          deliveryAddress: order.address || "",
          shippingCost: order.shippingCost ? Number(order.shippingCost) : 0,
          discount: order.totalDiscount ? Number(order.totalDiscount) : 0,
        });
      }).catch(() => {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Mude os detalhes ou converta esse orçamento
              </p>
            </div>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        append({ productId: "", productName: "", quantity: 1, unitPrice: 0, installmentPrice: 0 })
                      }
                    >
